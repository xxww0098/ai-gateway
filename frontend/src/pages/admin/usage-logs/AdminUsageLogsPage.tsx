      ...
      <div className="card px-5 py-4">
        <div className="flex flex-wrap items-end gap-3">
          <div className="min-w-[140px]">
            <label className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider mb-1 block">模型</label>
            <div className="relative">
              <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-muted-foreground pointer-events-none" />
              <input type="text" value={modelDraft} onChange={e => { setModelDraft(e.target.value) }}
                placeholder="搜索模型..." className="input h-9 text-sm pl-8"
                onKeyDown={e => { if (e.key === 'Enter') handleFilter() }}
              />
              {modelDraft && (
                <button onClick={() => { setModelDraft('') }} className="absolute right-2 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground">
          ...
          <div>
            <label className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider mb-1 block">状态</label>
          ...
          <div>
            <label className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider mb-1 block">范围</label>
              ...
              {([{ k: 'today' as const, l: '今天' }, { k: '7d' as const, l: '7天' }, { k: '30d' as const, l: '30天' }]).map(r => (
                <button key={r.k} onClick={() => { setDateRange(r.k); setPage(1) }}
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                    dateRange === r.k ? 'bg-primary text-primary-foreground shadow-xs' : 'bg-muted text-muted-foreground hover:bg-muted/80 hover:text-foreground'
                  }`}
                >{r.l}</button>
              ))}
            </div>
      ...
      <div className="rounded-xl border border-border bg-card overflow-hidden">
            ...
            <tbody>
              {logs.map(log => (
                ...
                >
                  ...
                  <td>
                    <span className="text-sm text-foreground truncate block max-w-[140px]" title={log.user_email}>
                      {log.user_email || `UID:${log.user_id}`}
                    </span>
                  </td>
                  <td>
                    <span className="text-sm text-muted-foreground truncate block max-w-[90px]">{log.api_key_name || '-'}</span>
                  </td>
                  <td>
                    <div className="flex items-center gap-1.5 flex-wrap">
                      <span className="font-mono text-xs font-medium text-foreground">{log.model}</span>
                      {log.provider && (
        ...
        {total > 0 && (
          <div className="px-5 py-3 border-t border-border flex items-center justify-between bg-muted/20">
            <div className="text-xs text-muted-foreground tabular-nums">共 {total.toLocaleString()} 条 · 第 {page}/{totalPages} 页</div>
            <div className="flex items-center gap-1.5">
              ...
              <button disabled={page <= 1} onClick={() => { handlePageChange(page - 1) }}
                className="h-8 w-8 rounded-lg border border-border bg-card flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-muted disabled:opacity-30 transition-colors">
