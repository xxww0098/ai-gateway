...
describe('DocsPage', () => {
  ...
  it('renders 接入指南 overview with gateway cards and three steps', () => {
    ...
    expect(text).toContain('查询额度')
    expect(text).toContain('GET /v1/usage')
    expect(text).toContain('balance_usd')
    expect(text).toContain('tokens_in')
  ...
  it('renders honest client pages and skips unknown slugs', () => {
    ...
    expect(textOf(mount('/docs/openai'))).toContain('GET /v1/usage')
    expect(textOf(mount('/docs/claude'))).toContain('Claude Code')
    expect(textOf(mount('/docs/claude'))).toContain('/v1/messages')
    expect(textOf(mount('/docs/claude'))).toContain('GET /v1/usage')
