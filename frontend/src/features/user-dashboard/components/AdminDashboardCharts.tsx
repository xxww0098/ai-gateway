    ...
    <div className="grid gap-6 lg:grid-cols-3 mt-6">
        ...
        <div className="p-5">
          {trendData.length === 0 ? (
            <EmptyState
              ...
              tone="no-results"
              icon={TrendingUp}
              title={`最近 ${trendDays} 天全站暂无调用记录`}
              description="可调整时间范围查看历史数据，或检查上游渠道连通状态。"
              action={{ label: '渠道管理', to: adminRoutes.channels }}
              className="h-[220px]"
            />
          ) : (
            <ResponsiveContainer width="100%" height={220}>
              <AreaChart data={trendData} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
                ...
                <Area yAxisId="left" type="monotone" dataKey="requests" stroke="#14b8a6" fill="#14b8a6" fillOpacity={0.08} strokeWidth={2} dot={false} activeDot={{ r: 4, strokeWidth: 0 }} />
                <Area yAxisId="right" type="monotone" dataKey="cost" stroke="#3b82f6" fill="#3b82f6" fillOpacity={0.08} strokeWidth={2} dot={false} activeDot={{ r: 4, strokeWidth: 0 }} />
              </AreaChart>
            </ResponsiveContainer>
          )}
          <div className="flex items-center justify-center gap-6 mt-3 text-xs text-muted-foreground">
            <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-[#14b8a6] inline-block" /> 请求数</span>
            <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-[#3b82f6] inline-block" /> 费用 ($)</span>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-border bg-card overflow-hidden">
        <div className="px-5 py-4 border-b border-border flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-foreground">模型分布</h3>
        </div>
        <div className="p-4">
          {modelData.length === 0 ? (
            <EmptyState
              ...
              icon={Flame}
              title="暂无模型用量数据"
              description="全站产生 API 调用后，此处将呈现各模型的调用量与费用分布。"
              action={{ label: '计费管理', to: adminBillingTab('pricing') }}
              className="h-[260px]"
            />
          ) : (
            <div className="flex flex-col items-center gap-4">
              <div className="relative w-[180px] h-[180px]">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                  <span className="text-lg font-bold tabular-nums text-foreground">{fmtCost(modelData.reduce((sum, m) => sum + m.cost, 0))}</span>
                  <span className="text-[10px] text-muted-foreground mt-0.5">总费用</span>
                </div>
              </div>
              <div className="w-full max-h-[120px] overflow-y-auto space-y-1 px-1">
                {modelData.slice(0, 8).map((m, i) => (
                  <div key={m.model} className="flex items-center justify-between text-[11px] group hover:bg-muted/40 rounded-md px-1.5 py-0.5 transition-colors">
                    <div className="flex items-center gap-1.5 min-w-0 flex-1">
                      <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: CHART_COLORS[i % CHART_COLORS.length] }} />
                      <span className="text-muted-foreground truncate" title={m.model}>{m.model}</span>
                    </div>
                    <div className="flex items-center gap-3 flex-shrink-0 ml-2">
                      <span className="text-muted-foreground tabular-nums">{m.requests}次</span>
                      <span className="text-foreground font-medium tabular-nums w-16 text-right">{fmtCost(m.cost)}</span>
                    </div>
                  </div>
                ))}
              </div>
