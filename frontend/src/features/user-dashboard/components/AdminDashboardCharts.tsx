import { TrendingUp, Flame, Server, Coins } from 'lucide-react'
import {
    ...
    <div className="rounded-[8px] border border-border bg-popover px-3.5 py-2.5 shadow-md text-xs text-popover-foreground min-w-[180px]">
      ...
      <div className="space-y-1">
        ...
        <div className="flex justify-between gap-4 border-t border-border pt-1">
          <span className="text-muted-foreground">精算费用</span>
      ...
      <div className="overflow-hidden rounded-[13px] border border-border bg-card shadow-[0_1px_2px_rgb(0_0_0/0.07)] lg:col-span-2 flex flex-col justify-between">
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-xs font-semibold text-foreground">调用量与费用走势</h3>
              <p className="text-[11px] text-muted-foreground">按日统计全站 API 转发频次与结算费用</p>
            </div>
          </div>
        ...
        <div className="p-4 flex-1 flex flex-col justify-center">
          {trendData.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-10 text-center">
              ...
              <h4 className="text-xs font-semibold text-foreground">最近 {trendDays} 天全站暂无调用</h4>
              <p className="mt-1 max-w-xs text-[11px] text-muted-foreground">
                上游渠道完成配置并由任意 agw- 密钥发起 /v1 请求后，此处将呈现请求与费用双轴曲线。
              </p>
              <Button asChild size="sm" variant="outline" className="mt-3.5 h-7 text-xs rounded-[7px]">
                <a href={adminRoutes.channels} className="inline-flex items-center gap-1">
            ...
            <>
              <ResponsiveContainer width="100%" height={240}>
                <AreaChart data={trendData} margin={{ top: 10, right: 10, left: -15, bottom: 0 }}>
                  ...
                  <Area yAxisId="left" type="monotone" dataKey="requests" stroke="#0d9488" fill="url(#adminReqGrad)" strokeWidth={2} dot={false} activeDot={{ r: 4, strokeWidth: 0 }} />
                  <Area yAxisId="right" type="monotone" dataKey="cost" stroke="#716dff" fill="url(#adminCostGrad)" strokeWidth={2} dot={false} activeDot={{ r: 4, strokeWidth: 0 }} />
                </AreaChart>
              </ResponsiveContainer>
      ...
      <div className="overflow-hidden rounded-[13px] border border-border bg-card shadow-[0_1px_2px_rgb(0_0_0/0.07)] flex flex-col justify-between">
        <div className="flex items-center justify-between border-b border-border px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-xs font-semibold text-foreground">模型调用分布</h3>
              <p className="text-[11px] text-muted-foreground">近 30 天各模型用量与费用占比</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 flex-1 flex flex-col justify-center">
          {modelData.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-10 text-center">
              ...
              <h4 className="text-xs font-semibold text-foreground">暂无模型用量数据</h4>
              <p className="mt-1 max-w-xs text-[11px] text-muted-foreground">
                网关产生实际调用后，将按各模型调用频次与费用展开饼图分布。
              </p>
              <Button asChild size="sm" variant="outline" className="mt-3.5 h-7 text-xs rounded-[7px]">
                <a href={adminBillingTab('pricing')} className="inline-flex items-center gap-1">
