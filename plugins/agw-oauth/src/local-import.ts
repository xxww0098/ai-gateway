...
export interface ImportReport {
  found: Array<Omit<LocalOauthFile, 'accessToken' | 'refreshToken' | 'idToken'> & { hasAccessToken: boolean, hasRefreshToken: boolean }>
  uploaded?: { count: number, status: number, body: unknown }
  error?: string
}
...
export function wellKnownPaths(home = homeDir()): string[] {
  const paths = [
  ...
  if (claudeDir) paths.push(join(claudeDir, '.credentials.json'))
  const grokDir = process.env.GROK_HOME?.trim()
  if (grokDir) paths.push(join(grokDir, 'auth.json'))
  return paths
}
...
function fromFlat(record: Record<string, unknown>, provider: LocalOauthFile['provider'], source: string): LocalOauthFile | undefined {
...
export function parseCliJson(raw: unknown, source: string): LocalOauthFile[] {
  if (raw === null || typeof raw !== 'object' || Array.isArray(raw)) return []
  const root = raw as Record<string, unknown>
  const out: LocalOauthFile[] = []

  if (root.claudeAiOauth !== null && typeof root.claudeAiOauth === 'object' && !Array.isArray(root.claudeAiOauth)) {
    const cred = fromFlat(root.claudeAiOauth as Record<string, unknown>, 'claude', source)
...
export async function discoverLocalOauth(home = homeDir()): Promise<LocalOauthFile[]> {
  const found: LocalOauthFile[] = []
...
export function redact(files: LocalOauthFile[]): ImportReport['found'] {
...
export function toUploadJson(file: LocalOauthFile): Record<string, unknown> {
  return {
    provider: file.provider,
...
export async function uploadToGateway(
  origin: string,
  apiKey: string,
  files: LocalOauthFile[],
...
export async function importLocalAndMaybeUpload(input: {
  ...
  const report: ImportReport = { found: redact(files) }
  if (files.length === 0) return report
  if (!input.origin || !input.apiKey) return report
  try {
