    ...
    async *stream(options) {
        ...
        if (model !== undefined && options.reasoningEffort !== undefined) {
            ...
            if (allowed.size === 0 || !allowed.has(options.reasoningEffort)) {
                throw new LlmError(`AI-GateWay model "${options.model}" does not support reasoning effort "${options.reasoningEffort}"`, 'UNSUPPORTED');
            }
        }
        const body = {
            model: options.model,
